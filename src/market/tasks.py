...
def sync_company_stock_quotes(company_id, days_ago=32, 
                              date_format = '%Y-%m-%d', verbose=False):
    Company = apps.get_model('market', 'Company')
    try:
        company_obj = Company.objects.get(id=company_id)
    except: company_obj = None
    if company_obj is None:
        raise Exception(f'{company_id} invalid')
    ...
    batch_insert_stock_data(dataset=dataset,
                            company_obj=company_obj, verbose=verbose)
